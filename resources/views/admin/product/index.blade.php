@extends('layouts.admin')

@section('content')
    <div class="container">
        <div class="row justify-content-center">
            @include('admin.aside')
            <div class="col-md-9">
                <div class="card shadow-lg border-0" style="background: #121212; color: #fff;">
                    <div class="card-header border-bottom border-secondary bg-transparent py-3 d-flex justify-content-between align-items-center">
                        <h5 class="m-0 text-uppercase fw-bold" style="letter-spacing: 2px; color: #00f2ea;">
                            Products //
                        </h5>
                        <a href="{{route('product.create')}}" class="btn btn-sm btn-outline-light rounded-0 text-uppercase fw-bold px-4">
                            + New Product
                        </a>
                    </div>

                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-dark table-hover mb-0 align-middle" style="background: transparent;">
                                <thead class="text-uppercase small text-muted border-bottom border-secondary">
                                <tr>
                                    <th class="py-3 ps-4" width="10%">Order</th>
                                    <th class="py-3" width="40%">Name</th>
                                    <th class="py-3" width="20%">Price</th>
                                    <th class="py-3 text-end pe-4">Actions</th>
                                </tr>
                                </thead>

                                <tbody>
                                @forelse($products as $item)
                                    <tr>
                                        <td class="ps-4 fw-bold text-secondary">#{{$item->position}}</td>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                @if($item->img)
                                                    <img src="/img/product/{{$item->img}}" class="rounded-circle border border-secondary me-3" width="40" height="40" style="object-fit:cover;">
                                                @endif
                                                <div>
                                                    <span class="fw-bold fs-6 d-block">{{$item->name}}</span>
                                                    <small class="text-muted text-uppercase" style="font-size: 0.7rem;">SKU: {{ $item->code ?? 'N/A' }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="text-success fw-bold">${{ number_format($item->price, 2) }}</span>
                                            <br>
                                            <small class="text-muted">Stock: {{ $item->stock }}</small>
                                        </td>
                                        <td class="text-end pe-4">
                                            <div class="btn-group" role="group">
                                                <a href="{{route('product.edit',$item->id)}}" class="btn btn-sm btn-outline-light rounded-0" title="Edit">
                                                    Edit
                                                </a>
                                                <form action="{{ route('product.destroy', $item->id) }}" method="POST" style="display:inline-block;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-sm btn-outline-danger rounded-0 border-start-0" onclick="return confirm('Are you sure you want to delete this product?')">
                                                        Delete
                                                    </button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center py-5 text-muted">
                                            <div class="mb-2" style="font-size: 2rem; opacity: 0.3;">🛍️</div>
                                            No products in inventory. Add your first item.
                                        </td>
                                    </tr>
                                @endforelse
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
