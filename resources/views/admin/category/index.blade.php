            ...
            <div class="col-md-9">
                <div class="card shadow-lg border-0" style="background: #121212; color: #fff;">
                    <div class="card-header border-bottom border-secondary bg-transparent py-3 d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{route('category.create')}}" class="btn btn-sm btn-outline-light rounded-0 text-uppercase fw-bold px-4">
                            + New
                        </a>
                    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end pe-4">
                                            <div class="btn-group" role="group">
                                                <a href="{{route('category.show',$item->id)}}" class="btn btn-sm btn-outline-info rounded-0" title="View Products">
                                                    Products
                                                </a>
                                                <a href="{{route('category.edit',$item->id)}}" class="btn btn-sm btn-outline-light rounded-0" title="Edit">
                                                    Edit
                                                </a>
                                                <form action="{{ route('category.destroy', $item->id) }}" method="POST" style="display:inline-block;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-sm btn-outline-danger rounded-0 border-start-0" onclick="return confirm('Are you sure?')">
                                                        Delete
                                                    </button>
                                                </form>
                                            </div>
